...
export const renderAllPlayers = (playerList) => {
  ...
  for (let i = 0; i < playerList.length; i++) {
    ...
    let pupHTML = `
      <div class="single-player-card">
        ...
        <button class="yeet-puppy" data-id=${pup.id}>YEET THIS PUPPY! (YOU MONSTER)</button>
      </div>
    `;
  ...
  let yeetPuppyBtn = document.getElementsByClassName("yeet-puppy");
  for (let puppyBtn of yeetPuppyBtn) {
    if (puppyBtn.dataset.id > 5800) {
      puppyBtn.addEventListener("click", async function () {
        // cl("Delete puppy ID of: " + puppyBtn.dataset.id);
        await removePlayer(puppyBtn.dataset.id);
  ...
  form.addEventListener("submit", async (event) => {
    ...
    */
    ...
    if (form.elements.imageUrl.value) {
      playerData.imageUrl = form.elements.imageUrl.value;
    }
